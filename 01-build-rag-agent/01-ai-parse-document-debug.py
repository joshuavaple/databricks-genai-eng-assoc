...
dbutils.widgets.text("volume", "rag")
...
# output table for the parsed results
output_table = f"{catalog}.{schema}.docs_parsed"

# COMMAND ----------

# MAGIC %md
# MAGIC ## A. `ai_parse_document` Usage
# MAGIC - Syntax: `ai_parse_document(content, map(<options>,<value>))`
# MAGIC - `content` is the only required argument. All options passed in the `map` argument (`version`, `imageOutputPath`, `descriptionElementTypes`, and `pageRange`) are optional.
# MAGIC   - content: A BINARY expression representing the input byte array data.
# MAGIC   - version: The version of the output schema, supported: "2.0".
# MAGIC   - 'imageOutputPath': Save rendered page images to a Unity Catalog volume for reference or multi-modal RAG applications.
# MAGIC   - 'descriptionElementTypes': AI-generated descriptions. Only descriptions for figures are supported for version 2.0.
# MAGIC   - 'pageRange': Restricts parsing to a subset of pages in the document. Page numbers are 1-indexed. The selected pages must stay within the 500-page limit. If pageRange is omitted and the document exceeds 500 pages, the function fails immediately without parsing any pages.
# MAGIC - Returns
# MAGIC   - "elements": list of different elements with their bbox, type, content, ect.
# MAGIC   - "pages": list of pages, their ids, and their parsed storage image paths in the volume
# MAGIC   - "metadata": document metadata
# MAGIC - For detailed I/O schemas, refer to: https://docs.databricks.com/aws/en/sql/language-manual/functions/ai_parse_document 

# COMMAND ----------

# MAGIC %md
# MAGIC ### A1. Parsing with Python

# COMMAND ----------

from pyspark.sql.functions import expr

# read all files from the document volume. Each file occupies 1 row.
# columns: ['path', 'modificationTime', 'length', 'content']
docs_df = spark.read.format("binaryFile").load(source_files)

# parse each doc binary content to a new column "parsed_content" using ai_parse_document (a SQL expression)
parsed_df = docs_df.withColumn("parsed_content", expr(
  f"""
...
# drop the original binary content column
...
#       )
#     ) as parsed_doc
#   FROM read_files('{source_files}', format => 'binaryFile')
# """)
...
# MAGIC %md
# MAGIC ### B1. Helper Class Definition
# MAGIC These helpers read the parsed elements of the document and render them with HTML within the Notebook environment with a pre-defined style.
...
# MAGIC %md
# MAGIC Display all pages in a specified range

# COMMAND ----------

# # Select a sample document and render its parsed content
# sample = parsed_df.select("parsed_content").limit(1).collect()

# if sample:
#   doc = sample[0]["parsed_content"]
#   render_ai_parse_output(doc, page_selection="1-5")
# else:
...
# MAGIC %md
# MAGIC display all pages one by one interactively

# COMMAND ----------

# DBTITLE 1,Debug Visualization Results
# render_ai_parse_output_interactive(parsed_df)
if sample:
  doc = sample[0]["parsed_content"]
  render_ai_parse_output_interactive(doc)
else:
  print("No parsed documents found.")
...
# MAGIC Persist the table containing parsed elements for all documents in a delta table

# COMMAND ----------

try:
  parsed_df.write.format("delta").mode("overwrite").saveAsTable(output_table)
  print(f"✅ Parsed results saved to table {output_table}")
except Exception as e:
  print(f"❌ Failed to save parsed results to table {output_table} with the following error:")
  print(e)
